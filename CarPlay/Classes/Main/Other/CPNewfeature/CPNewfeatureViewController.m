...
#define XMNewfeatureImageCount 3
...
- (void)setupScrollView
{
    ...
    for (int i = 0; i<XMNewfeatureImageCount; i++) {
        ...
        if ([UIScreen mainScreen].bounds.size.height == 480.0) { // 4inch  需要手动去加载4inch对应的-568h图片
...
- (void)setupStartButton:(UIImageView *)imageView
{
    ...
    // 3.设置frame
    startButton.size = CGSizeMake(self.view.width/2, 50);
    startButton.centerX = self.view.width * 0.5;
    startButton.centerY = self.view.height * 0.27;
