...
@interface CPBRPGViewController ()<UIGestureRecognizerDelegate>
{
    UIView *corentView;
...
@property (nonatomic, strong) UIButton *lastPayBtn;
@property (nonatomic , strong) CPActivityModel *activity;

@end

@implementation CPBRPGViewController
...
- (void)viewDidLoad {
    ...
    _lastPayBtn = self.treatBtn;
    [self.footBallBtn setSelected:YES];
    [self.treatBtn setSelected:YES];
...
- (IBAction)payBtnClick:(CPBRPGMatchingSelectButton *)sender {
    if (!sender.selected) {
