...
- (void)writePngWithFileName:(NSString *)name atomically:(BOOL)atomically
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [UIImagePNGRepresentation(self) writeToFile:name.cachePath atomically:atomically];
    });
}
...
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [UIImageJPEGRepresentation(self, quality) writeToFile:name.cachePath atomically:atomically];
    });
}

+ (UIImage *)imageWithFileName:(NSString *)name
{
    NSData *data = [NSData dataWithContentsOfFile:name.cachePath];
    if (data) {
